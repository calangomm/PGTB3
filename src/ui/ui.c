...
int initialize_window(SDL_Window **window, SDL_Renderer **renderer, TTF_Font **font, int nivel) {
    ...
    case 2:
    case 3:
        w_height = WINDOW_HEIGHT+WINDOW_HEIGHT_LVL_2_3_DIF;
        break;
    }
    // Criar janela SDL
...
void render(SDL_Renderer *renderer, Jogo *jogo, TTF_Font *font) {
    ...
    if (jogo->nivel>1)
    {
        // Cria o bloco do tabuleiro
        draw_rectangle(renderer, DIST_HEADER_LEFT, DIST_TABULEIRO_TOP, 
                    HEADER_WIDTH, TABULEIRO_HEIGHT, 
                    C_TABULEIRO, true);
    }
    ...
    for (int i = 0; i < 7; i++) {
        ...
        switch (i) {
            ...
            case 1:
                ...
                break;
            default:
                deck = &jogo->fundacao[i - 3];
                break;
        }

        if(i!=2)
        {
            if (deck->indice > 0)desenha_carta_header(renderer, &deck->cartas[deck->indice - 1], "../assets/images/cards", cartas_desenho.x, cartas_desenho.y);
            else draw_rectangle(renderer, cartas_desenho.x, cartas_desenho.y, CARD_WIDTH, CARD_HEIGHT, C_CARDS_BACK, true);
        }
        

        cartas_desenho.x += CARD_WIDTH + CARDS_DIST;
    }   
    ...
    //desenha o tabuleiro
    //itera o array da pilhas
    for(int i= 0; i <NUM_PILHAS; i++)
    {
        cartas_desenho.y = DIST_TABULEIRO_TOP;
        cartas_desenho.x = DIST_HEADER_LEFT+(i*(CARD_WIDTH+CARDS_DIST));

        //itera o array do de deck da pilha
        for(int j = 0; j< jogo->pilhas[i].indice; j++)
        {
            desenha_carta_header(renderer,&jogo->pilhas[i].cartas[j],
                                    "../assets/images/cards", cartas_desenho.x, cartas_desenho.y);
            cartas_desenho.y = cartas_desenho.y + DIST_CARDS_BARALHO;
        }
